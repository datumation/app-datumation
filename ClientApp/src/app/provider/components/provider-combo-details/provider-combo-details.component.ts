...
@Component({
  selector: 'app-provider-combo-details',
  templateUrl: './provider-combo-details.component.html',
  styleUrls: ['./provider-combo-details.component.css']
})
export class ProviderComboDetailsComponent implements OnInit {
public providerInfo;
attemptPurchase: boolean;
purchaseConfirm: boolean;
messagePrompt: boolean;
constructor(private route: ActivatedRoute, private router: Router, public auth: AuthService, private http: HttpClient, @Inject('BASE_URL') _baseUrl: string) {
  ...
  ngOnInit() {
    ...
    const provider = {
      'stateName': this.route.queryParams['value']['providerState'],
      'specialtyType': this.route.queryParams['value']['providerType']
    };
