...
@Component({
  selector: 'app-provider-state-details',
  templateUrl: './provider-state-details.component.html',
  styleUrls: ['./provider-state-details.component.css']
})
...
export class ProviderStateDetailComponent implements OnInit {
public providerInfo;
...
constructor(private route: ActivatedRoute, private router: Router, public auth: AuthService, private http: HttpClient, @Inject('BASE_URL') _baseUrl: string) {
  ...
  ngOnInit() {
    ...
    const provider = {
      ...
      'specialtyType': 'COMPLETE FOR STATE'
    };
